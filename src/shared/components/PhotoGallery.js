...
const Picture = ({ picture, handleOnPress, handleImageLongPress, isSelectedToDelete }) => {
  return (
    <TouchableOpacity onLongPress={() => handleImageLongPress(picture)} onPress={() => handleOnPress(true)} style={styles.thumbContainer}>
      <Image borderRadius={4} style={styles.picture} source={{ uri: picture.uri }} />
      {isSelectedToDelete && <View style={styles.selectedOverlayContainer}></View>}
    </TouchableOpacity>
  )
}
...
class PhotoGallery extends React.Component {
  ...
  static navigationOptions = ({ navigation }) => {
    ...
    return {
      ...
      headerLeft: <TouchableOpacity onPress={() => isEditing ? cancelEditing() : navigation.goBack()} style={{ paddingHorizontal: 29 }}>
        <MaterialCommunityIcons name={isEditing ? 'close' : "chevron-left"} size={38} color="#707070" />
      </TouchableOpacity>,
      headerRight: isEditing && <TouchableOpacity onPress={finishDelete} style={{ paddingHorizontal: 29, flexDirection: 'row', justifyContent: 'center', alignItems: 'center' }}>
  ...
  isSelectedToDelete = picture => {
    const images = this.props.navigation.getParam('images', [])
    return images.some(x => x === picture.id)
  }

  addImageToDelete = picture => {
    let images = this.props.navigation.getParam('images', [])

    if (images.some(x => x === picture.id)) {
      images = images.filter(x => x !== picture.id)
      this.props.navigation.setParams({
        images: images,
    ...
    return (
      <ScrollView
        ...
        contentContainerStyle={styles.contentContainerStyle}>
        {
          pictures.length
            ? <React.Fragment>
              <TouchableOpacity onPress={this.handleOnPictureAdd} style={styles.addPictureContainer}>
                <MaterialCommunityIcons name="camera" size={38} color="#707070" />
              </TouchableOpacity>
              {pictures.map((picture, index) => <Picture
                picture={picture}
                key={index}
                isSelectedToDelete={this.isSelectedToDelete(picture)}
                handleImageLongPress={picture => this.handleImageLongPress(picture)}
                handleOnPress={value => isEditing ? this.addImageToDelete(picture) : this.handleGalleryOpen(value)} />)}
            </React.Fragment>
            : <PhotoGalleryEmptyState onPictureAdd={this.handleOnPictureAdd} />
        }
        <Carousel caption={caption} isOpen={isGalleryOpen} handleOpen={this.handleGalleryOpen} pictures={pictures} />
        <ImageSelector
